...
async function handleDialogFlowAction(
  ...
  switch (action) {
    ...
    case "ArtistaPrendaEspecifica.action":
      sendTextMessage(
        sender,
        "tenemos disponibles las siguientes prendas de " +
    ...
    case "ArtistaPrendaYTalla.action":
      if (
        parameters.fields.Talla.stringValue == "" ||
        parameters.fields.NombreDePrenda.stringValue == "" ||
        parameters.fields.Prenda.stringValue == ""
      ) {
        handleMessages(messages, sender);
        break;
      }
      ChatBotUsers_Products.saveUserInterest(
        sender,
        parameters.fields.NombreDePrenda.stringValue,
        parameters.fields.Prenda.stringValue
      );
      let res = await ProductDescriptions.getPrice(
        parameters.fields.Talla.stringValue,
        parameters.fields.NombreDePrenda.stringValue,
        parameters.fields.Prenda.stringValue
      );
    ...
    case "DatosRecibidos.action":
      if (
        parameters.fields.phoneNumber.stringValue != "" &&
        parameters.fields.email.stringValue != ""
      ) {
...
async function sendTextMessage(recipientId, text) {
  if (text.includes("{first_name}")) {
    ...
    text = text.replace("{first_name}", userData.first_name);
  }
  var messageData = {
    recipient: {
      id: recipientId,
