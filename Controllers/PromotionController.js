...
let savePromotion = async (req, res) => {
    try {
        ...
        let promotion = await createPromotion(data.description, data.picture, data.discount);
        for (let j = 0; j < products.length; j++) {
            const element = products[j];
            await Promotions_Products.createPromotionsProducts(element.productName, element.productType, promotion);
        }
        let activeClients = await ChatBotUsers.getChatBotUserByState(4);
        for (let i = 0; i < activeClients.length; i++) {
            const element = activeClients[i];
            await Messages.saveAndSendMessage(element.facebookId, data.description, data.picture);
        }
        res.json("ok");
    } catch (error) {
        console.log(error);
